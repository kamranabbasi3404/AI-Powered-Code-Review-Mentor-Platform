      ...
      {showLogoutConfirm && (
        <div className="modal-overlay" onClick={cancelLogout}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            ...
            <p style={{ marginBottom: '1.5rem', color: 'var(--text-secondary, #666)' }}>Are you sure to logout?</p>
            <div style={{ display: 'flex', gap: '1rem', justifyContent: 'flex-end' }}>
              <button className="btn" onClick={cancelLogout} style={{ background: 'transparent', border: '1px solid var(--border)', color: 'var(--text-primary)' }}>No</button>
              <button className="btn btn-primary" onClick={confirmLogout}>Yes</button>
            </div>
          </div>
        </div>
      )}
